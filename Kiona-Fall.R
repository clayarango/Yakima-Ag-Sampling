...
d.sum$site_date<-"kiona_fall"
write.table(d.sum, "kiona_fall_summ.csv",  sep=",", quote=F, row.names=F)
...
qqline(E1)
ad.test(E1)
   #residuals are normally distributed p = 0.1173
hist(E1)  
plot(M1)
...
qqline(E1)
ad.test(E1)
#residuals are normally distributed p = 0.40
hist(E1)  
plot(M1)
...
qqline(E1)
ad.test(E1)
  #residuals are normal (p = 0.1307)
hist(E1)
plot(M1)

bartlett.test(chla~nutrient, data=d.gpp)
  #variance is OK p = 0.3619
...
qqline(E1)
ad.test(E1)
#residuals look good (p=0.20)
hist(E1)
plot(M1)

bartlett.test(chla~nutrient, data=d.gpp)
#data look good p=0.36
