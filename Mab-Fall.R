...
x
d.cr$cr.nrr = d.cr$cr.area/-21.727991 #is there a prettier way to do this?

#calculate nrr for gpp
x = aggregate(d.gpp[,9], list(d.gpp$nutrient), mean, na.rm=T)
x
d.gpp$gpp.nrr = d.gpp$gpp.area/3.3947683 #is there a prettier way to do this?
...
bartlett.test(cr.area~nutrient, data=d.cr)
   #variance test is bad, but close
