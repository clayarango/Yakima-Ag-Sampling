...
d.sum$site_date<-"mab_fall"
write.table(d.sum, "mab_fall_summ.csv",  sep=",", quote=F, row.names=F)
...
qqline(E2)
ad.test(E2)
#residuals are not normal, p=1.49e-7
hist(E2)  
plot(M2)
...
#variance test is not equal, p=3.356e-13
...
qqline(E3)
ad.test(E3)
#residuals are not normally distributed p = 0.003406
hist(E3)  
plot(M3)
...
qqline(E1)
ad.test(E1)
#residuals are normally distributed p = 0.086
hist(E1)  
plot(M1)
...
qqline(E2)
ad.test(E2)
#residuals are not normally distributed p = 0.038
hist(E2)  
plot(M2)
...
qqline(E3)
ad.test(E3)
#residuals are not normally distributed p = 0.045
hist(E3)  
plot(M3)
...
qqline(E1)
ad.test(E1)
#residuals look good (p=0.6374)
hist(E1)
plot(M1)

bartlett.test(chla_ug_cm2~nutrient, data=d.gpp)
#data look good p=0.23
