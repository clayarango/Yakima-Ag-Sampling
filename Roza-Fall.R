...
#N limitation, possible P+Si secondary limitation
...
ggplot(data=subset(d.gpp, !(nutrient=="control")), aes(x=nutrient, y=chla.nrr))+geom_boxplot()+theme_bw()+
  ...
  theme(axis.title.x=element_blank(), panel.grid.minor=element_blank(), panel.grid.major=element_blank())
#possible N limitation (but not NP), possible Si secondary limitation
...
d.sum$site_date<-"roza_fall"
write.table(d.sum, "roza_fall_summ.csv",  sep=",", quote=F, row.names=F)
...
bartlett.test(cr.area~nutrient, data=d.cr)
   #variance test is not equal, p=0.02481
...
qqline(E2)
ad.test(E2)
#residuals are not normal, p=0.0003489
hist(E2)  
plot(M2)
...
#variance test is not equal, p=4.8e-5
...
qqline(E3)
ad.test(E3)
#residuals are not normally distributed p = 0.00546
hist(E3)  
plot(M3)
...
qqline(E1)
ad.test(E1)
#residuals are normally distributed p = 0.08
hist(E1)  
plot(M1)

bartlett.test(cr.area~nutrient, data=d.crNoSi)
#variance test 0.02, we'll probably need to mess with this one
...
qqline(E2)
ad.test(E2)
#residuals are not normally distributed p = 0.001
hist(E2)  
plot(M2)
...
qqline(E1)
ad.test(E1)
#residuals look passable (p=0.049)
hist(E1)
plot(M1)

bartlett.test(chla~nutrient, data=d.gpp)
#data look bad p=0.019
...
qqline(E2)
ad.test(E2)
#residuals are passable, p=0.047
...
interaction.plot(xx$P, xx$N, xx$chla)
#No limiation
