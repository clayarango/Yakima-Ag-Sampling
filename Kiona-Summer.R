...
d.nrr$site.date<-"kiona_summer"
d.nrr$gpp.es<-log(d.nrr$gpp.nrr)
d.nrr$cr.es<-log(d.nrr$cr.nrr)
d.nrr$chla.es<-log(d.nrr$chla.nrr)

write.table(d.nrr, "kiona_summer_nrr.csv",  sep=",", quote=F, row.names =F)
...
#maybe N+P colimitation, but probably no limitation
...
d.sum$site_date<-"kiona_summer"
write.table(d.sum, "kiona_sum_summ.csv",  sep=",", quote=F, row.names=F)
...
qqline(E1)
ad.test(E1)
#residuals are normally distributed, p=0.1266
hist(E1)  
plot(M1)
...
qqline(E1)
ad.test(E1)
#residuals are normally distributed, p=0.2372
hist(E1)  
plot(M1)
...
qqline(E1)
ad.test(E1)
#residuals are normally distributed, p=0.06483
hist(E1)  
plot(M1)
...
qqline(E1)
ad.test(E1)
  #residuals are normal (p = 0.2976)
hist(E1)
plot(M1)

bartlett.test(chla~nutrient, data=d.gpp)
  #variance is OK p = 0.117
