...
d.nrr$site.date<-"cen_lan_summer"
d.nrr$gpp.es<-log(d.nrr$gpp.nrr)
d.nrr$cr.es<-log(d.nrr$cr.nrr)
d.nrr$chla.es<-log(d.nrr$chla.nrr)

write.table(d.nrr, "cen_lan_summer_nrr.csv",  sep=",", quote=F, row.names =F)
...
ggplot(data=subset(d.gpp, !(nutrient=="control")), aes(x=nutrient, y=gpp.nrr))+geom_boxplot()+theme_bw()+
  ...
  theme(axis.title.x=element_blank(), panel.grid.minor=element_blank(), panel.grid.major=element_blank())
#possible N+P colimitation and Si limitation
...
d.sum$site_date<-"cen_lan_summer"
write.table(d.sum, "cen_lan_summer_summ.csv",  sep=",", quote=F, row.names=F)
...
qqline(E1)
ad.test(E1)
  #residuals are normally distributed p = 0.3354
hist(E1)  
plot(M1)
...
qqline(E1)
ad.test(E1)
#residuals are normally distributed, p=0.5259
hist(E1)  
plot(M1)
...
qqline(E1)
ad.test(E1)
#residuals are normally distributed, p=0.7471
hist(E1)  
plot(M1)
...
qqline(E1)
ad.test(E1)
#residuals are normally distributed, p=0.05119
hist(E1)  
plot(M1)
...
anova(M1) #no limitation (Si somewhat suppressive)
