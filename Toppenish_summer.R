...
qqline(E1)
ad.test(E1)
   #residuals are more-or-less normally distributed, p=0.2851.Some wiggle around QQ line
hist(E1) #ok
plot(M1)# ok
...
M1<-gls(gpp.area~N*P*Si, data=d.gpp, na.action=na.omit) #had to log-transform
E1<-residuals(M1)
qqnorm(E1)
qqline(E1)
ad.test(E1)
  #ok, a little wiggle, p = 0.49
